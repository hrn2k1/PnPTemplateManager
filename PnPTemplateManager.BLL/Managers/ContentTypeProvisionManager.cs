...
using PnPContentType= OfficeDevPnP.Core.Framework.Provisioning.Model.ContentType;

namespace PnPTemplateManager.Managers
{
    public class ContentTypeProvisionManager
    {

        public TokenParser Provision(Web web, ProvisioningTemplate template, TokenParser parser, ProvisioningTemplateApplyingInformation applyingInformation)
        {
            ...
            foreach (var ct in template.ContentTypes.OrderBy(ct => ct.Id)) // ordering to handle references to parent content types that can be in the same template
            {
                ...
                if (existingCT == null)
                {
                    Debug.WriteLine("Creating content type {0} {1} to site columns", ct.Id, ct.Name);
                    //scope.LogDebug(CoreResources.Provisioning_ObjectHandlers_ContentTypes_Creating_new_Content_Type___0_____1_, ct.Id, ct.Name);
                    var newCT = CreateContentType(web, ct, parser, template.Connector ?? null, existingCTs, existingFields, isNoScriptSite);
                    if (newCT != null)
                    {
                ...
                else
                {
                    if (ct.Overwrite)
                    {
                        ...
                        Debug.WriteLine("Recreating content type {0} {1} to site columns", ct.Id, ct.Name);
                    ...
                    {
                        Debug.WriteLine("Updating content type {0} {1} to site columns", ct.Id, ct.Name);
                        ...
                        UpdateContentType(web, existingCT, ct, parser, isNoScriptSite);
                    }
                }
            }
        ...
        public ProvisioningTemplate Extract(Web web, ProvisioningTemplateCreationInformation creationInfo)
        {

            ProvisioningTemplate template = new ProvisioningTemplate();
            template.ContentTypes.AddRange(GetEntities(web, creationInfo, template));
        ...
        private IEnumerable<PnPContentType> GetEntities(Web web, ProvisioningTemplateCreationInformation creationInfo, ProvisioningTemplate template)
        {
            var cts = web.ContentTypes;
            web.Context.Load(cts, ctCollection => ctCollection.IncludeWithDefaultProperties(ct => ct.FieldLinks));
            web.Context.ExecuteQueryRetry();

            List<PnPContentType> ctsToReturn = new List<PnPContentType>();
            ...
            foreach (var ct in cts)
            {
                if (!BuiltInContentTypeId.Contains(ct.StringId))
                {
                    ...
                    PnPContentType newCT = new PnPContentType
                        (ct.StringId,
                        ct.Name,
                        ct.Description,
                        ct.Group,
                    ...
                    if (creationInfo.PersistMultiLanguageResources)
                    {
                        ...
                        if (persistLanguages)
                        {
                            if (PersistResourceValue(ct.NameResource, string.Format("ContentType_{0}_Title", ct.Name.Replace(" ", "_")), template, creationInfo))
                            {
                                newCT.Name = string.Format("{{res:ContentType_{0}_Title}}", ct.Name.Replace(" ", "_"));
                            }
                            if (PersistResourceValue(ct.DescriptionResource, string.Format("ContentType_{0}_Description", ct.Name.Replace(" ", "_")), template, creationInfo))
                            {
        ...
        private ProvisioningTemplate CleanupEntities(ProvisioningTemplate template, ProvisioningTemplate baseTemplate)
        {
            foreach (var ct in baseTemplate.ContentTypes)
            {
                ...
                {
                    Debug.WriteLine("Creating content type {0} {1} to template", ct.Id, ct.Name);
    ...
    public class ContentTypeIdToken : TokenDefinition
    {
